...
业务用例 = **"用户来到这个系统想完成的一件事"**（goal-level），不是"系统提供的一个功能"。AI 默认会按特性/功能切分，必须按下面的规则收敛。

**拆分必要条件（两条都满足才拆）**：

1. 价值接收者不同（不同 actor 或不同用户群）
2. 用户描述目的时的口径明显不同

**合并的情形（不要拆）**：

- 同一目标的多种实现 / 增强 —— 例：rewind、跨设备续、导出留存都属于"远程用 AI"一个目标
- 同一目标的不同阶段 —— 例：开会话 / 发消息 / 中断 / 关闭都在一个目标里

**剥离的情形（根本不属于业务用例）**：

- 技术运维（adduser、launchd kickstart、监控、迁移脚本）—— 是部署/运维事项，不进 business_use_cases
- 系统内部触发的非用户感知行为（jsonl 监听、定时器、自愈逻辑）—— 是 server 内部应用用例

**经验阈值**：典型工程 1~5 条业务用例；>8 条几乎肯定是在罗列功能特性而不是目标。超阈值时回头按"卖测试"和"goal vs feature"重新归并。

**命名导向**：`name` 应表达"为什么来这个系统"（差异化的价值主张），不只是"做什么"（动词宾语，缺差异化）。

| 弱 | 强 | 差别 |
|----|----|------|
| 查看文件信息 | 免 SSH 查看远端文件 | 后者体现了"相对替代方案的价值" |
| 管理项目 | 在手机上恢复 PC 上的工作流 | 前者纯活动名，后者点出场景痛点 |
...
### business_workers vs external_parties 的判断（AI 常见误区）

| | 定义 | 例 |
|--|------|-----|
| `business_workers` | **组织内部、为运营系统而设的角色** | chargable-proxy 的 Admin（公司员工，运营客服） |
| `external_parties` | **系统的服务对象 / 集成方 / 数据源** | chargable-proxy 的 Player（客户）、Taobao（集成平台） |

判断口诀：**这个角色是组织雇来跑系统的，还是来用系统拿价值的？**

**单人工具 / 自研内部工具的特例**：`business_workers` 通常为 `[]`。开发者本人是 `external_parties`（系统的服务对象，即使本人也维护代码），不是 `business_workers`——把"使用"当"运营"是常见误用。
