...
同时注意全局与项目级的归属：
- **`~/.claude/CLAUDE.md` 保持极简**：只放极少数纯个人偏好（如语言偏好、审美），不要塞工作规范——它作用于所有 Claude 会话（含非工程类对话），堆多了会污染日常对话
- **跨项目通用的工作规范**（工作模式、知识库结构、代码规范等）通过 `/aie-apply` 推到每个被管工程的项目级 `CLAUDE.md`，边界清晰、可按项目调整
- **项目特有的设置**留在项目级配置，不应跑到全局去影响其他项目
