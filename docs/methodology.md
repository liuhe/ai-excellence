...
### 对话焦点维护 — 对抗 lost in the middle

长上下文对开头和结尾敏感，中段信息容易被弱化（lost in the middle）。一次长对话里丢的往往不是事实，而是**对话脉络**——当前焦点、概念定义、已排除方向。历史消息的位置不可控，能控的是"模型当前以什么理解在回答"。

- **信号触发，不定期总结**：跑偏不按节奏发生，定时/定轮总结大多浪费 token，且总结本身也膨胀上下文。有效信号：用户纠偏语、同一点被连续追问、回答变泛、引用已废弃方向
- **纠偏即复述**：用户纠偏的那一刻就是触发点——先用一句话复述修正后的焦点，确认后再继续。不纠偏说明没跑偏，不需要总结
- **机制落位**：触发规则写进项目 CLAUDE.md（常驻约束，见 `/aie-apply` A 节"对话焦点维护"）；UserPromptSubmit hook 每轮注入一句判断提醒——只注入判断规则，不注入内容摘要，否则 hook 自身就在制造膨胀；人工只保留"感觉不对就喊停"的兜底
